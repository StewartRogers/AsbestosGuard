...
app.get('/api/policies', async (req, res) => {
    try {
        ...
        if (!fs.existsSync(docsDir))
            return res.json({ policies: [], combinedText: '' });
...
app.get('/api/applications', async (req, res) => {
    try {
        if (USE_AZURE_STORAGE) {
            ...
            const applications = await Promise.all(blobNames.map(async (blobName) => {
...
app.get('/api/fact-sheets', async (req, res) => {
    try {
        if (USE_AZURE_STORAGE) {
            ...
            const factSheets = await Promise.all(blobNames.map(async (blobName) => {
                const filename = blobName.replace('.json', '');
...
app.get('/api/analysis', async (req, res) => {
    try {
        if (USE_AZURE_STORAGE) {
            ...
            const analyses = await Promise.all(blobNames.map(async (blobName) => {
                const filename = blobName.replace('.json', '');
...
app.post('/__api/gemini/analyze', async (req, res) => {
    try {
        ...
        try {
            ...
            const analyzeApplication = mod?.analyzeApplication ?? mod?.default?.analyzeApplication;
