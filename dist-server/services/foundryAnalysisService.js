 ...
 */
export async function analyzeApplication(application, factSheet) {
    try {
 ...
 */
function buildAnalysisPrompt(application, factSheet) {
    const wizard = application.wizardData || {};
 ...
 */
function parseAgentResponse(application, agentResponse, factSheet) {
    let parsed;
    try {
        ...
        console.error('[foundryAnalysisService] Parse error:', error.message);
    ...
    const mapRecommendation = (rec) => {
        ...
        if (lower.includes('reject'))
            return 'REJECT';
        if (lower.includes('request') || lower.includes('info'))
            return 'REQUEST_INFO';
