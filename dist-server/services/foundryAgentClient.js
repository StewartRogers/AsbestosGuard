...
async function getAuthToken() {
    ...
    if (!token?.token)
        throw new Error('Failed to acquire Azure AI Foundry token');
    return token.token;
}
...
export async function createThread() {
    for (const apiVersion of API_VERSIONS) {
        try {
            ...
            console.log(`createThread failed with ${apiVersion}: ${err.message}`);
        }
    }
...
export async function addMessage(threadId, message) {
    for (const apiVersion of API_VERSIONS) {
        try {
            ...
            console.log(`addMessage failed with ${apiVersion}: ${err.message}`);
        }
    }
...
export async function runThread(threadId, assistantId) {
    for (const apiVersion of API_VERSIONS) {
        try {
            ...
            console.log(`runThread failed with ${apiVersion}: ${err.message}`);
        }
    }
...
export async function getRun(threadId, runId) {
    for (const apiVersion of API_VERSIONS) {
        try {
            ...
            console.log(`getRun failed with ${apiVersion}: ${err.message}`);
        }
    }
...
export async function getMessages(threadId) {
    for (const apiVersion of API_VERSIONS) {
        try {
            ...
            console.log(`getMessages failed with ${apiVersion}: ${err.message}`);
        }
    }
    throw new Error('Failed to get messages with any supported API version');
}
export async function askAgent(assistantId, prompt, opts) {
