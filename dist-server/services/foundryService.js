...
function getAgentNameOrId(agentKey) {
    const map = {
    ...
    if (!nameOrId)
        throw new Error(`Missing Foundry agent name/ID for key '${agentKey}'. Set FOUNDRY_AGENT_${agentKey.slice(-1)}_ID or FOUNDRY_AGENT_${agentKey.slice(-1)}_NAME.`);
    return nameOrId;
}
export async function chatWithAgent(agentKey, prompt) {
