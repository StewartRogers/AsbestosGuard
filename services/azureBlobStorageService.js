...
function getBlobServiceClient() {
    ...
    try {
        ...
        if (AZURE_STORAGE_ACCOUNT && AZURE_STORAGE_KEY) {
            ...
            blobServiceClient = new BlobServiceClient(`https://${AZURE_STORAGE_ACCOUNT}.blob.core.windows.net`, sharedKeyCredential);
        ...
        if (AZURE_STORAGE_ACCOUNT) {
            ...
            blobServiceClient = new BlobServiceClient(`https://${AZURE_STORAGE_ACCOUNT}.blob.core.windows.net`, defaultCredential);
 ...
 */
export async function uploadBlob(containerName, blobName, data) {
    try {
 ...
 */
export async function listBlobs(containerName) {
    try {
        const containerClient = await getContainerClient(containerName);
        const blobNames = [];
 ...
 */
export async function blobExists(containerName, blobName) {
    try {
        const containerClient = await getContainerClient(containerName);
        const blockBlobClient = containerClient.getBlockBlobClient(blobName);
...
export const saveApplication = (filename, data) => uploadBlob(APPLICATIONS_CONTAINER, `${filename}.json`, data);
export const loadApplication = (filename) => downloadBlob(APPLICATIONS_CONTAINER, `${filename}.json`);
export const listApplications = () => listBlobs(APPLICATIONS_CONTAINER);
export const deleteApplication = (filename) => deleteBlob(APPLICATIONS_CONTAINER, `${filename}.json`);
// Fact sheet-specific helpers
export const saveFactSheet = (filename, data) => uploadBlob(FACT_SHEETS_CONTAINER, `${filename}.json`, data);
export const loadFactSheet = (filename) => downloadBlob(FACT_SHEETS_CONTAINER, `${filename}.json`);
export const listFactSheets = () => listBlobs(FACT_SHEETS_CONTAINER);
export const deleteFactSheet = (filename) => deleteBlob(FACT_SHEETS_CONTAINER, `${filename}.json`);
// Analysis-specific helpers
export const saveAnalysis = (filename, data) => uploadBlob(ANALYSIS_CONTAINER, `${filename}.json`, data);
export const loadAnalysis = (filename) => downloadBlob(ANALYSIS_CONTAINER, `${filename}.json`);
export const listAnalyses = () => listBlobs(ANALYSIS_CONTAINER);
export const deleteAnalysis = (filename) => deleteBlob(ANALYSIS_CONTAINER, `${filename}.json`);
// Generic data storage helpers
export const saveData = (key, data) => uploadBlob(DATA_CONTAINER, `${key}.json`, data);
export const loadData = (key) => downloadBlob(DATA_CONTAINER, `${key}.json`);
export const deleteData = (key) => deleteBlob(DATA_CONTAINER, `${key}.json`);
// Policy document helpers (for .docx files)
export const savePolicyDocument = (filename, buffer) => uploadBlob(POLICIES_CONTAINER, filename, buffer);
export const loadPolicyDocument = (filename) => downloadBlob(POLICIES_CONTAINER, filename);
export const listPolicyDocuments = () => listBlobs(POLICIES_CONTAINER);
