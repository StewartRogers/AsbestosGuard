...
let validateAiAnalysis;
try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const AjvMod = (await import('ajv')).default || (await import('ajv'));
    const ajv = new AjvMod({ allErrors: true, strict: false });
    ...
    validateAiAnalysis = (_) => true;
}
// Basic input validation for incoming requests
function validateApplicationInput(application) {
    if (!application)
        throw new Error('Application is required');
    if (!application.companyName || !application.companyName.trim())
        throw new Error('Company name is required');
    if (!application.address || !application.address.trim())
        throw new Error('Address is required');
}
// Type guard for AIAnalysisResult to provide runtime checks
function isValidAIAnalysisResult(obj) {
    if (!obj || typeof obj !== 'object')
        return false;
    const allowed = ['LOW', 'MEDIUM', 'HIGH', 'INVALID'];
    if (!('riskScore' in obj) || !allowed.includes(obj.riskScore))
        return false;
    if (typeof obj.summary !== 'string')
        return false;
    if (!obj.internalRecordValidation || typeof obj.internalRecordValidation !== 'object')
        return false;
    if (!obj.geographicValidation || typeof obj.geographicValidation !== 'object')
        return false;
    if (!obj.webPresenceValidation || typeof obj.webPresenceValidation !== 'object')
        return false;
    return true;
}
export const analyzeApplication = async (application, factSheet) => {
    ...
    if (!nodeModel) {
        try {
            console.warn('GEMINI_MODEL not set; falling back to default model', CONFIG.DEFAULT_MODEL);
        }
        catch (e) { }
    ...
    function createFallbackAnalysis(opts) {
        const { riskScore = 'MEDIUM', isTestAccount = false, summary = 'AI returned an unexpected response. Manual review required.', concerns = ['AI response parsing failed.'], recommendation = 'MANUAL_REVIEW_REQUIRED', factSheetSummary = 'Analysis unavailable.', webPresenceSummary = 'Analysis unavailable.', debugPrompt = '', rawResponse = '', extraDebug = undefined, preserve = {} } = opts || {};
        const base = {
            riskScore,
            isTestAccount,
            summary,
            ...
            debug: Object.assign({ prompt: debugPrompt || '', rawResponse: rawResponse || '' }, extraDebug || {})
        };
        // Optional user-facing summaries
        base.factSheetSummary = opts.factSheetSummary || factSheetSummary;
        base.webPresenceSummary = opts.webPresenceSummary || webPresenceSummary;
        return base;
    }
    ...
    if (typeof window !== 'undefined') {
        ...
        async function tryProxyEndpoints(endpoints, payload, timeoutMs = CONFIG.DEFAULT_TIMEOUT_MS) {
            let lastError = null;
    ...
    async function loadPoliciesText() {
        try {
            if (typeof window !== 'undefined') {
                try {
    ...
    try {
        ...
        async function generateAndParse(modelPrompt) {
            ...
            try {
                ...
                if (!text && anyResp.candidates && anyResp.candidates.length)
                    text = anyResp.candidates.map((c) => c.text || JSON.stringify(c)).join('\n');
                ...
                if (finish === 'MAX_TOKENS') {
                    try {
                        ...
                        if (!contText && a.candidates && a.candidates.length)
                            contText = a.candidates.map((c) => c.text || JSON.stringify(c)).join('\n');
                        if (contText)
                            text = String(text || '') + '\n' + String(contText);
                    }
                    catch (e) {
...
`;
        ...
        if (!parsedFact) {
            try {
                ...
                const ts = Date.now();
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent1-fact-raw-${ts}.txt`), `FACT:\n${factText}`, 'utf8');
                }
                catch (e) { }
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent1-fact-response-${ts}.json`), JSON.stringify({ factResp: factResp.response }, null, 2), 'utf8');
                }
                catch (e) { }
            }
            catch (e) { /* ignore */ }
        ...
        if (!parsedPolicy) {
            try {
                ...
                const ts = Date.now();
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent2-policy-raw-${ts}.txt`), `POLICY:\n${policyText}`, 'utf8');
                }
                catch (e) { }
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent2-policy-response-${ts}.json`), JSON.stringify({ policyResp: policyResp.response }, null, 2), 'utf8');
                }
                catch (e) { }
            }
            catch (e) { /* ignore */ }
        ...
        if (!parsedWeb) {
            try {
                ...
                const ts = Date.now();
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent3-web-raw-${ts}.txt`), `WEB:\n${webText}`, 'utf8');
                }
                catch (e) { }
                try {
                    fs.writeFileSync(pathMod.join(dir, `agent3-web-response-${ts}.json`), JSON.stringify({ webResp: webResp.response }, null, 2), 'utf8');
                }
                catch (e) { }
            }
            catch (e) { /* ignore */ }
        ...
        const agentStatuses = [];
        if (parsedFact)
            agentStatuses.push('internal record validation');
        if (parsedPolicy)
            agentStatuses.push('policy compliance');
        if (parsedWeb)
            agentStatuses.push('web presence verification');
        const failedAgents = [];
        if (!parsedFact)
            failedAgents.push('internal record');
        if (!parsedPolicy)
            failedAgents.push('policy analysis');
        if (!parsedWeb)
            failedAgents.push('web search');
        ...
        const riskPriority = { 'HIGH': 3, 'MEDIUM': 2, 'LOW': 1, 'INVALID': 0 };
        merged.riskScore = riskScores.reduce((highest, current) => (riskPriority[current] || 0) > (riskPriority[highest] || 0) ? current : highest, failedAgents.length > 0 ? 'MEDIUM' : 'LOW');
        ...
        var response = { candidates: [{ groundingMetadata: { groundingChunks: [] } }] }; // placeholder for grounding
        ...
        function extractFirstJson(input) {
        ...
        function parseLargestMatch(raw) {
        ...
        if (!parsedCandidate) {
            ...
            try {
                ...
                const ts = Date.now();
                try {
                    fs.writeFileSync(pathMod.join(dir, `raw-${ts}.txt`), String(text || ''), 'utf8');
                }
                catch (e) { /* ignore */ }
                try {
                    fs.writeFileSync(pathMod.join(dir, `response-${ts}.json`), JSON.stringify(response || {}, null, 2), 'utf8');
                }
                catch (e) { /* ignore */ }
        ...
        try {
            ...
            try {
                ...
                const ts = Date.now();
                try {
                    fs.writeFileSync(pathMod.join(dir, `normalize-error-${ts}.txt`), String(text || ''), 'utf8');
                }
                catch (e) { /* ignore */ }
                try {
                    fs.writeFileSync(pathMod.join(dir, `normalize-response-${ts}.json`), JSON.stringify(response || {}, null, 2), 'utf8');
                }
                catch (e) { /* ignore */ }
        ...
        (function ensureRequiredTopLevel(obj) {
            try {
                if (!obj)
                    return;
                const allowedRisk = ['LOW', 'MEDIUM', 'HIGH', 'INVALID'];
                if (!obj.riskScore || !allowedRisk.includes(obj.riskScore))
                    obj.riskScore = 'MEDIUM';
                if (!obj.summary || typeof obj.summary !== 'string')
                    obj.summary = 'Partial AI output — manual review recommended.';
                if (!obj.recommendation || typeof obj.recommendation !== 'string')
                    obj.recommendation = 'MANUAL_REVIEW_REQUIRED';
        ...
        try {
            const valid = validateAiAnalysis(result);
            if (!valid) {
                const validationErrors = validateAiAnalysis.errors;
                ...
                return createFallbackAnalysis({
                    riskScore: (result && result.riskScore) || 'MEDIUM',
                    isTestAccount: !!result?.isTestAccount,
                    summary: (result && result.summary) || 'Partial AI output — manual review required.',
